...
if [ "$CUSTOM_CONFIG" = "true" ]; then
  ...
  for _qn in qn_protocol_byte qn_report_byte qn_live_weight_ack auto_clear_stale_bond; do
    if [ -n "$(opt "$_qn")" ]; then
...
YAML
  ...
  if [ -n "$SCALE_MAC" ] || [ -n "$BLE_ADAPTER" ] || [ -n "$FORCE_SCALE_ADAPTER" ]     || [ -n "$QN_PROTOCOL_BYTE" ] || [ -n "$QN_REPORT_BYTE" ] || [ -n "$QN_LIVE_WEIGHT_ACK" ]     || [ "$AUTO_CLEAR_STALE_BOND" = "true" ] || [ "$PROXY_LIVENESS_MIN" != "30" ]; then
    echo "ble:" >> "$FRESH"
    ...
    [ -n "$QN_LIVE_WEIGHT_ACK" ] && echo "  qn_live_weight_ack: $QN_LIVE_WEIGHT_ACK" >> "$FRESH"
